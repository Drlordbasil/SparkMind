...
class AIChatbot:
    ...
    def run_program(self):
        process = subprocess.Popen(
            ["python", self.program_name], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    ...
    def start_chat(self):
        ...
        while True:
            ...
            if response == "Error: Missing dependencies. Install the required libraries - requests, transformers, bs4":
                print(
                    "AI Chatbot: Please install the missing dependencies before running the program.")
                break

            if response.startswith("Error:"):
                print(
                    "AI Chatbot: An error occurred while running the program. Please check the script and try again.")
                break
            ...
            confirmation = input(
                "AI Chatbot: Do you want to run the program again? (Y/N): ")
